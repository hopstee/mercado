...
use Larapen\LaravelLocalization;
...
function lurl($path = null, $attributes = [], $locale = null)
{
	...
	$localized = (new LaravelLocalization\LaravelLocalization())->getLocalizedURL($locale, $path, $attributes);
 ...
 */
function phoneFormat($phone, $countryCode = null ,$format = \libphonenumber\PhoneNumberFormat::INTERNATIONAL)
{
	// Country Exception
	if ($countryCode == 'UK') {
		$countryCode = 'GB';
...
function getPhoneIcon($countryCode)
{
	if (file_exists(public_path() . '/images/flags/16/' . strtolower($countryCode) . '.png')) {
		...
		$phoneIcon = '<i class="unir-phone"></i>';
	}
...
function getLoginLabel()
{
	if (isEnabledField('email') && isEnabledField('phone')) {
		$loginLabel = t('Phone or Username');
	} else {
...
function genEmailContactBtn($post = null, $btnBlock = false, $iconOnly = false)
{
	...
	if (!isset($post->phone) || empty($post->phone)) {
		if ($iconOnly) {
			$out = '<i class="icon-mail" style="color: #dadada"></i>';
		}
...
/**
 * Spaces between contry code, operator
 * R.S
 * @param $phone
 */
