...
class ConversationsController extends AccountBaseController
{
	...
	public function index()
	{
		...
		foreach($data['conversations'] as $kk => $conversation){
			// R.S
			//if logged user is not initiator of conversation
			if( $conversation->from_user_id != auth()->user()->id){
				$conversation->new_messages = Message::where('parent_id', $conversation->id )
				->where('to_user_id', auth()->user()->id )
				->where('is_read', 0 )
				->count();
				$childrenMessage = Message::where('parent_id', $conversation->id)->count();

				if( $conversation->new_messages == 0 && $childrenMessage == 0){
					$parentMessage = Message::where('is_read', 0 )				
					->where('to_user_id', auth()->user()->id )
					->where('id', $conversation->id )
					->count();
					$conversation->new_messages += $parentMessage;
				}
			}
			else{
				$conversation->new_messages =  (Message::where('id', $conversation->id )
				->where('to_user_id', auth()->user()->id )
				->where('is_read', 0 )
				->count() > 0)
				? (Message::where('id', $conversation->id )
				->where('to_user_id', auth()->user()->id )
				->where('is_read', 0 )
				->count())
				: Message::where('parent_id', $conversation->id )
				->where('to_user_id', auth()->user()->id )
				->where('is_read', 0 )
				->count();
			}
	...
	public function messages($conversationId)
	{
		...
		if ($conversation->is_read != 1) {
			if ($data['countMessages'] > 0) {
				...
				// R.S
				foreach($data['messages'] as $kk => $message){
					if($message->is_read == 0 && $message->from_user_id != auth()->user()->id){
							$message->is_read = 1;
							$message->save();
					}
				}
	...
	public function reply($conversationId, ReplyMessageRequest $request)
	{
		...
		if ($conversation->from_user_id != auth()->user()->id) {
			...
			$toName = isset($conversation->from_name) ? $conversation->from_name : $conversation->from_phone;
...
//			. '<br><br>'
//			. t('Related to the ad')
			. ' | ' . UrlGen::post($conversation->post);

		// R.S.
		// Save
		try{
			$message->save();
			flash(t("Your message has sent successfully to :contact_name.", ['contact_name' => $message->to_name ]))->success();
		}catch(\Exception $e){
