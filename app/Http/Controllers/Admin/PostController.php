...
//		]);
    ...
    public function createCategoryTree($languageCode, $parent)
    {
        $result = [];
        ...
        foreach ($subCats as $sub) {
            ...
            $subs = $this->createCategoryTree($languageCode, $sub->id);
            if (count($subs) > 0) {
                $node['nodes'] = $subs;
    ...
    {
        $cacheId = 'moderate.categories';
        $subCats = Cache::remember($cacheId, new \DateInterval("PT30M"), function () {
            $subCats = $this->createCategoryTree('en', 0);
            return $subCats;
        });
