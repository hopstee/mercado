...
class SubCategoryController extends PanelController
{
    ...
    public function setup()
    {
        ...
        $_id = request()->segment(5);
        // Get Parent Category name
        $parent = Category::findTransOrFail($this->parentId);
        $_entry = ($_id > 0) ? Category::findTransOrFail($_id) : null;
        ...
        */
        ...
        if (!isset($_entry)) {
            ...
            $this->xPanel->setEntityNameStrings(
                trans('admin::messages.subcategory') . ' &rarr; ' . '<strong>' . $_entry->name . '</strong>',
                trans('admin::messages.subcategories') . ' &rarr; ' . '<strong>' . $_entry->name . '</strong>'
            );
        }


        $this->xPanel->enableReorder('name', 0);
        ...
        $top_category = $temp->getMyParentID();
        if ($top_category == 0) {
            $this->xPanel->setParentRoute(admin_uri('categories'));
        } else {
            $this->xPanel->setParentRoute(admin_uri('categories/' . $top_category . '/subcategories'));
        }
