...
class Post extends BaseModel implements Feedable
{
	...
	*/
	...
	public function getTitleHtml()
	{
		...
		// R.S
		if($post->archived == 1){
			$deleted = '<div style="color: red;">*Archived</div>';
			$out .= $deleted;
		}
	...
	public function getUserNameHtml()
	{
		if (isset($this->user) and !empty($this->user)) {
			...
			// R.S
			if($this->user->closed == 1  && $this->user->id != 1 ){
				$deleted = '<div style="color: red;">*Deleted</div>';
				$this->contact_name .= $deleted;
			}
			if(!is_null(Blacklist::where('entry',$this->user->phone)->first())){
				$blocked = '<div style="color: red;">*Banned</div>';
				$this->contact_name .= $blocked;
			}
	...
	//E.K.
	public function getReviewedPosts()
        {
                return ajaxReviewedDisplay($this->{$this->primaryKey}, $this->getTable(), 'reviewed', $this->reviewed);
        }
