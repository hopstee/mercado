...
function listView(selecter) {
	...
	if ($('.adds-wrapper').hasClass('property-list')) {
		...
		$('.item-list .add-desc-box').removeClass("col-md-9").addClass("col-md-10");
	}
...
function compactView(selecter) {
	...
	if ($('.adds-wrapper').hasClass('property-list')) {
		...
		$('.item-list .add-desc-box').addClass("col-md-9").removeClass('col-md-10');
	}
...
function checkNewMessages() {
	...
	var oldValue = $('.count-conversations-with-new-messages').html();
	...
	$.ajax({
		...
		if (data.countConversationsWithNewMessages > 0) {
			...
			$('.count-conversations-with-new-messages').addClass('show-badget');
			$('.new-messages').addClass('show-badget');
			...
			$('.count-conversations-with-new-messages').removeClass('show-badget');
			$('.new-messages').removeClass('show-badget');
		}
